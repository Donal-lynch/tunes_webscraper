...
def scrape_page(driver, url, directory):
    
    # Get the info which relates to the while page
    page = requests.get(url)

    soup = BeautifulSoup(page.content, 'html.parser')

    page_data = get_meta_data(soup, -1)
    
    # How many versions are on this page?
    num_versions = len(soup.find_all(class_="notes"))
    
    # Now cycle through each of the versions and download the file and get the
    # meta for each version
    
    driver.get(url)
    
    for i in range(num_versions):
        
        version_data = get_meta_data(soup, i)
                
        get_ith_mid_file(driver, i)
        
        
        new_file_name = page_data[0] + ' v' + str(version_data[0]) + ' ' + str(version_data[3]) + '.mid'
        
        rename_newsest_file(directory, new_file_name)
...
def get_ith_mid_file(driver, index):
